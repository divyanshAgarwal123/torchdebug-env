...
# Ensure common Docker binary locations are discoverable (especially on macOS).
...
if [ -f "$REPO_DIR/Dockerfile" ]; then
  ...
  DOCKER_ARGS=("-f" "$DOCKERFILE" "$DOCKER_CONTEXT")
...
BUILD_OK=false
BUILD_OUTPUT=$(run_with_timeout "$DOCKER_BUILD_TIMEOUT" docker build "${DOCKER_ARGS[@]}" 2>&1) && BUILD_OK=true
